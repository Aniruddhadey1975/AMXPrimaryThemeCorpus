...
search_tab, advance_search_tab , executive_dashboard_tab , brand_dashboard_tab, industry_dashboard_tab, people_dashboard_tab, competitive_dashboard_tab  = st.tabs(["Search","Advance Search" , "Executive Dashboard", "Brand Dashboard","Industry Dashboard", "People Dashboard","Competitive Dashboard"])
...
# search results block, display only if results are found
                ...
                with search_results:
                    ...
                    # print(len(mergedArticles. columns))
                    # print(mergedArticles.head(50))
                    ...
                    with articles_list:
                        ...
                        for _, a in mergedArticles.head(50).iterrows():
                            ...
                            with stats_col:
                                    ...
                                    with sent_stat_row:
                                        try:
                                            print("I am at line number 311.")
                                            print(type(a['articleSentiment']))
                                            # bxxx=a['display_content'][:100]
                                            # print(bxxx)
                                            # print(type(bxxx))
                                            # proposed_method_sentiment=calc.custom_sentiment(bxxx)
                                            # print(proposed_method_sentiment)
                                    ...
                                    with reach_ave_syndication_row:
                                        ...
                                        # article_syn, article_syn_str = calc.convert_to_millions_or_thousands(a['Syndication'])
                                        # print(type(article_reach))
                                        # print(type(article_reach_str))
                                        # print(type(article_ave))
                                        # print(type(article_ave_str))
                                        st.write("Reach ",article_reach_str,"\t" ,  " AVE", article_ave_str, "\t", "Syndication",a['Syndication'])
                                    # print(len(a['Syndication']))
                                        # st.write("Syndication")       
        ...
        if mergedArticles.empty:
            ...
            if view_exec_dashboard:
                ...
                with executive_dashboard_view:
                    ...
                    sentiment_counts = calc.get_total_sentiments()
                    ...
                    # sentiment_pos_kpi.metric(label="Positive", value=sent_pos_str, delta= round(total_articles) - 10)
                    # sentiment_neg_kpi.metric(label="Negative", value=sent_neg_str, delta= round(total_articles) - 10)
                    # sentiment_neu_kpi.metric(label="Neutral", value=sent_neu_str, delta= round(total_articles) - 10)
    ...
    if brand_dashboard_visible:
        ...
        with vol_ana:
            ...
            with vol_anac1:
                ...
                vol_articles.metric(label="Total Articles ", value=total_articles_str, delta= round(total_articles) - 10)



                vol_reach.metric(label="Total Reach ", value=total_reach_str, delta= round(total_articles) - 10)



                vol_ave.metric(label="Total AVE ", value=total_ave_str, delta= round(total_articles) - 10)
            ...
            with sent_ana:
                ...
                with sent_anac1:
                    ...
                    # sent_pos_kpi.metric(label="Positive", value=sent_pos_str, delta= round(total_articles) - 10)
                    # sent_neg_kpi.metric(label="Negative", value=sent_neg_str, delta= round(total_articles) - 10)
                    # sent_neu_kpi.metric(label="Neutral", value=sent_neu_str, delta= round(total_articles) - 10)
...
################### Industry DASHBOARD starts downward ###############################################
with industry_dashboard_tab:
    # initialize 

    industry_dashboard_visible = False

    st.header("AlphametricX Industry Dashboard")
    
    industry_dashboard = st.container()

    with industry_dashboard:
        st.header("Industry dashboard view")


    if mergedArticles.empty:
        st.write("Please make a search")
    else:
        industry_dashboard_visible = True

    if industry_dashboard_visible:
        vol_ana_sent_ana = st.container()
        ...
        with vol_ana:
            ...
            vol_anac1,vol_anac2 = st.columns([1,1])
            # print(type(vol_anac1))
            # print(type(vol_anac2))
            # print(len(vol_anac1))
            # print(len(vol_anac2))
            # print(type())
            # print(type())
            with vol_anac1:
                ...
                # print(len(total_reach))
                # print(len(total_reach_str))
                total_ave, total_ave_str = calc.get_total_ave()
        ...
        with media_type_dist_reach_over_time:
            ...
            with reach_over_time:
                ...
                reach_over_time_data_expander = st.expander(label="Reach over time")
    st.write("Results for Industry Dashboard are Under Progress.")
################### People DASHBOARD starts downward ###############################################
with people_dashboard_tab:
    st.write("Results for People Dashboard are Under Progress.")
################### EXECUTIVE DASHBOARD END ###############################################
with competitive_dashboard_tab:
    st.write("Results for Competitive Dashboard are Under Progress.")
